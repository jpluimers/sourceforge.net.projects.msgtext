...
/* crc lookup table */
ULONG aulCrc32[] = {      0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419,
...
/*--------------------------------------------------------------------*/
void Crc::digest(BYTE c)
{
  ULONG ulByte = 0;
  memcpy(&ulByte,&c,1); /* avoids the signed/unsigned problems */
  int iIndex = (m_ulCrc ^ ulByte) & 0x000000FF;
  m_ulCrc = aulCrc32[iIndex] ^ (m_ulCrc >> 8);
} /* digest */
