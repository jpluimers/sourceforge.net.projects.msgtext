/*-- $Workfile: Property.cpp $ -----------------------------------------
Wrapper for an SPropValue.
Version    : $Id$
Application: MSG file archiving
Platform   : Windows
Description: Property class wraps an SPropValue.
------------------------------------------------------------------------
...
/*--------------------------------------------------------------------*/
LPOLESTR Property::getSubstgName()
{
  LPOLESTR awcFORMAT = L"__substg1.0_%08x";
   /* "HHHHHHHH" instead of "%08x" plus 0 terminator: */
  LPOLESTR pwcsStreamName = (LPOLESTR)malloc(2*(wcslen(awcFORMAT)+5));
  wsprintf(pwcsStreamName,awcFORMAT,m_ulTag);
  return pwcsStreamName;
} /* getSubstgName */

/*--------------------------------------------------------------------*/
Property::Property(Storage* pStorage, ULONG ulTag, void* pValue)
{
  m_ulTag = ulTag;
  memcpy(m_acValue,pValue,sizeof(m_acValue));
  m_pStorage = pStorage;
} /* PropValue */
...
char* Property::type()
{
  ...
  case PT_MV_I2: psType = "PT_MV_I2"; break;
  // case PT_MV_SHORT: psType="PT_MV_SHORT"; break;
  case PT_MV_I4: psType = "PT_MV_I4"; break;
  // case PT_MV_LONG: psType = "PT_MV_LONG"; break;
  case PT_MV_I8: psType = "PT_MV_I8"; break;
  // case PT_MV_LONGLONG: psType = "PT_MV_LONGLONG"; break;
  case PT_MV_R4: psType = "PT_MV_R4"; break;
  // case PT_MV_FLOAT: psType = "PT_MV_FLOAT"; break;
  case PT_MV_R8: psType = "PT_MV_R8"; break;
  // case PT_MV_DOUBLE: psType = "PT_MV_DOUBLE"; break;
  case PT_MV_CURRENCY: psType = "PT_MV_CURRENCY"; break;
  case PT_MV_APPTIME: psType = "PT_MV_APPTIME"; break;
  case PT_MV_SYSTIME: psType = "PT_MV_SYSTIME"; break;
  case PT_MV_STRING8: psType = "PT_MV_STRING8"; break;
  case PT_MV_UNICODE: psType = "PT_MV_UNICODE"; break;
  case PT_MV_BINARY: psType = "PT_MV_BINARY"; break;
  case PT_MV_CLSID: psType = "PT_MV_CLSID"; break;
...
char* Property::value()
{
  ...
  Heap* pHeap = new Heap();
#endif
  sprintf(acValue,"0x%08x",getLongValue());
  ...
  case PT_NULL: break;
  case PT_I2: sprintf(acValue,"%hd",getShortValue()); break;
  case PT_I4: sprintf(acValue,"%lu",getLongValue()); break;
  case PT_I8: sprintf(acValue,"0x%016x",(LPBYTE)&getLargeValue()); break;
  case PT_R4: sprintf(acValue,"%f",getFloatValue()); break;
  case PT_R8: sprintf(acValue,"%f", getDoubleValue()); break;
  case PT_CURRENCY: break;
  case PT_APPTIME: break;
  case PT_ERROR: break;
  case PT_BOOLEAN: sprintf(acValue,"%s",getBooleanValue() == false? "false": "true"); break;
  ...
  if (!(pHeap->isValid() && pHeap->hasInitialSize()))
    printf("Property::value: Heap inconsistent after Property::value()!\n");
  delete pHeap;
#endif
  return psValue;
} /* value */
...
BOOL Property::variable()
{
  ...
  {
  case PT_OBJECT: ; break;
  case PT_STRING8: ; break;
  case PT_UNICODE: ; break;
  case PT_CLSID: ; break;
  case PT_BINARY: ; break;
  case PT_MV_STRING8: ; break;
  case PT_MV_UNICODE: ; break;
  case PT_MV_CLSID: ; break;
  case PT_MV_BINARY: ; break;
  default: bVariable = false;
  }
  return bVariable;
} /* variable */

/*--------------------------------------------------------------------*/
BOOL Property::multiValued()
{
  BOOL bMultiValued = false;
  USHORT usPropType = (USHORT)(this->tag() & 0x0000FFFF);
  if ((usPropType & MV_FLAG) == MV_FLAG)
    bMultiValued = true;
  return bMultiValued;
} /* multiValued */
...
/*--------------------------------------------------------------------*/
BOOL Property::getBooleanValue()
{
  BOOL bValue = false;
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if (usPropType == PT_BOOLEAN)
  {
    bValue = true;
    if (getShortValue() == 0)
      bValue = false;
...
/*--------------------------------------------------------------------*/
short Property::getShortValue()
{
  USHORT usValue = -1;
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if ((usPropType == PT_BOOLEAN) || (usPropType == PT_I2))
    memcpy(&usValue,m_acValue,sizeof(usValue));
  return usValue;
} /* getShortValue */

/*--------------------------------------------------------------------*/
ULONG Property::getLongValue()
{
  ULONG ulValue = -1;
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if ((usPropType == PT_I4) || variable())
    memcpy(&ulValue,m_acValue,sizeof(ulValue));
  return ulValue;
} /* getLongValue */

/*--------------------------------------------------------------------*/
LARGE_INTEGER Property::getLargeValue()
{
  ...
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if (usPropType == PT_I8)
    memcpy(&liValue,m_acValue,sizeof(liValue));
  return liValue;
} /* getLargeValue */
...
float Property::getFloatValue()
{
  ...
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if (usPropType == PT_R4)
    memcpy(&fValue,m_acValue,sizeof(fValue));
  return fValue;
} /* getFloatValue */
...
double Property::getDoubleValue()
{
  ...
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if (usPropType == PT_R8)
    memcpy(&dValue,m_acValue,sizeof(dValue));
  return dValue;
} /* getDoubleValue */

/*--------------------------------------------------------------------*/
FILETIME Property::getSysTimeValue()
{
  FILETIME ftValue;
  ftValue.dwHighDateTime = 0;
  ftValue.dwLowDateTime = 0;
  USHORT usPropType = (USHORT)(this->tag() & 0x00000FFF);
  if (usPropType == PT_SYSTIME)
    memcpy(&ftValue,m_acValue,sizeof(FILETIME));
  return ftValue;
} /* getSysTimeValue */
...
/*--------------------------------------------------------------------*/
Stream* Property::getBinaryStream()
{
  /* make name "__substg1.0_HHHHHHHH" and get its content from Storage */
  LPOLESTR pwcsStreamName = getSubstgName();
  Stream* pBinaryStream = m_pStorage->getStream(pwcsStreamName);
  free(pwcsStreamName);
  return pBinaryStream;
} /* getBinaryStream */

/*--------------------------------------------------------------------*/
IStream* Property::getBinaryIStream()
{
  /* make name "__substg1.0_HHHHHHHH" and get its content from Storage */
  LPOLESTR pwcsStreamName = getSubstgName();
  IStream* pBinaryIStream = m_pStorage->getIStream(pwcsStreamName);
  free(pwcsStreamName);
  return pBinaryIStream;
} /* getBinaryIStream */
...
/*--------------------------------------------------------------------*/
IStorage* Property::getBinaryIStorage()
{
  /* make name "__substg1.0_HHHHHHHH" and get its content from Storage */
  LPOLESTR pwcsStorageName = getSubstgName();
  IStorage* pBinaryStorage = m_pStorage->getIStorage(pwcsStorageName);
  free(pwcsStorageName);
  return pBinaryStorage;
} /* getBinaryStorage */

/*--------------------------------------------------------------------*/
HRESULT Property::list()
{
  HRESULT hr = NOERROR;
